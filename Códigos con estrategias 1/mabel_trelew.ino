...
int msMov = 0;
#define T_GIRO_10 10
#define T_GIRO_90 200
#define T_GIRO_180 (T_GIRO_90 * 2)  // tiempo estimado para 180 grados
#define T_RECTO 200
#define PMW_BUSQ_LENTO 100
#define PMW_BUSQ_RAP 200  //aca lo tengo que cambiar si quiero que vaya mas rapido
#define PMW_ATAQUE_LENTO 100
#define PMW_ATAQUE_RAP 220
#define PMW_GIRO 150
...
//////////////SENSORES////////////////
//Distancia
#define DIST_CERCA 30  //ver mañana si hay que subirlo para que detecte mas
//Derecha
...
//Piso
//Derecha
#define SENS_PISO_DER 4
bool pisoDer = false;
//Izquierda
#define SENS_PISO_IZQ 5
bool pisoIzq = false;
//Blanco y negro
#define BLANCO 90
bool leerPiso(int);
...
void loop() {
  ...
  //Piso
  // Lectura directa del piso
  // Lectura directa del piso
  valorPisoDer = analogRead(SENS_PISO_DER);
  valorPisoIzq = analogRead(SENS_PISO_IZQ);

  // Detecta borde (blanco)
  if (valorPisoDer <= BLANCO || valorPisoIzq <= BLANCO) {
    estado = NOS_CAEMOS;
  }
  estados();
}
  ...
  void estados() {
    switch (estado) {
      ...
      case NOS_CAEMOS:
        ...
        Serial.print(" | ");
        Serial.println(valorPisoIzq);

        if (valorPisoDer <= BLANCO && valorPisoIzq > BLANCO) {
          // Piso derecho detectado → retrocede y gira izquierda 180°
          ...
          motorB_atras(PMW_ATAQUE_RAP);
          delay(T_GIRO_180);
        } else if (valorPisoDer > BLANCO && valorPisoIzq <= BLANCO) {
          // Piso izquierdo detectado → retrocede y gira derecha 180°
          motorA_atras(PMW_ATAQUE_RAP);
          motorB_atras(PMW_ATAQUE_RAP);
          delay(300);
          motorA_atras(PMW_ATAQUE_RAP);
          ...
          // Ambos detectan → retrocede y gira 180°
          motorA_atras(PMW_ATAQUE_RAP);
          motorB_atras(PMW_ATAQUE_RAP);
          delay(300);
          motorA_atras(PMW_ATAQUE_RAP);
  ...
  void antiReboteBotonDer(void) {
    ...
    switch (maqBotonDer) {
      ...
      case CONFIRMACION_BOT_DER:
        estadoBotonDer = analogRead(PIN_BOTON_DER);
  ...
  void antiReboteBotonIzq(void) {
    ...
    switch (maqBotonIzq) {
      ...
      case CONFIRMACION_BOT_IZQ:
        estadoBotonIzq = analogRead(PIN_BOTON_IZQ);
  ...
  void centrar(int pmw) {
    if ((distIzq == true) && (distCen == false)) {
      // Si el sensor izquierdo detecta, ahora debe girar hacia la izquierda
      motorA_adelante(pmw);
      motorB_atras(pmw);
      Serial.println("izq");
    } else if ((distDer == true) && (distCen == false)) {
      // Si el sensor derecho detecta, ahora debe girar hacia la derecha
      motorA_atras(pmw);
      motorB_adelante(pmw);
      Serial.println("der");
    }
  ...
  /*bool leerPiso(int SENS_PISO) {
  int piso = analogRead(SENS_PISO);
  Serial.print("Sensor "); Serial.print(SENS_PISO); Serial.print(" = "); Serial.println(piso);
  return piso <= BLANCO;  // ajustalo después de medir
}*/
